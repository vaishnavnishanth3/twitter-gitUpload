...
export async function followUnfollowUser(req, res) {
    try{
        ...
        console.log("Error in followers Controller: ", error.message)
        res.status(500).json({ error : "Internal Server Error"})
    }
}

export async function updateUserProfile(req, res) {
    const { fullName, username, currentPassword, email, newPassword, bio, link} = req.body
    ...
    try{
        ...
        if (currentPassword && newPassword) {
            ...
            if (newPassword.length < 6) {
                return res.status(400).json({ error : "Password must be 6 characters atleast"})
            }
